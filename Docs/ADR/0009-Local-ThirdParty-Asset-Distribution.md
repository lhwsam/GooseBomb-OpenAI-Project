...
`Assets/ThirdParty`에는 UI 에셋 공급자가 배포한 원본 파일이 들어 있다. 게임에서 사용할 권리가 있더라도 공개 Git 저장소에 원본 파일을 그대로 재배포할 권리와는 다를 수 있다. 반대로 단순히 폴더만 삭제하면 로비와 pause UI의 Sprite 참조가 깨져 외부 에셋이 없는 개발자와 자동 검증 환경이 프로젝트를 정상적으로 열 수 없다.
...
- Editor validator는 모든 `Assets/Game` 직접 의존성과 공개 대체 바인딩을 검사한다.
...
- `Assets/ThirdParty`는 Git 기록에서 제거되지만 기존 비공개 커밋의 객체는 cleanup commit만으로 사라지지 않는다. 저장소 공개 전 별도 history purge와 새 clone 검증이 필요하다.
