...
- 추격자 이동, 폭탄 설치, 실제 fuse 폭발과 플레이어 자기 피해·적 사망·방 클리어, 교체, pause/resume.
- 사용자 입력 뒤 오디오 재생.
- 전체 화면/창 크기 변경 시 화면과 UI.
- 브라우저 Console error와 WebGL context loss.
- 캐시된 이전 버전에서 새 버전 갱신.

개발 빌드 자동 probe는 `probe-ready`로 런타임 준비를 동기화한 뒤 `move`, `chaser-moved`, `place-bomb`, `bomb-exploded`, `player-damaged`, `enemy-died`, `room-cleared`, `swap-bomb`, `pause-resume`, `audio-unlocked`를 확인한다. `move`와 `chaser-moved`는 성공한 Core actor 셀 전이, `place-bomb`은 성공한 Core 설치, `bomb-exploded`는 fuse 뒤 확정된 Core 폭발, 피해·사망·클리어 표식은 논리 영향 셀과 체력 규칙을 통과한 실제 결과에서 발생한다. 실제 보간 좌표, pooled 표현 생명주기, 피격 material property block은 PlayMode 통합 테스트가 확인한다. `audio-unlocked` marker는 오디오 출력 자체를 증명하지 않으므로 오디오 연결 뒤 수동 항목을 별도로 통과해야 한다.
