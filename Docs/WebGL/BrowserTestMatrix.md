...
개발 빌드 자동 probe는 `probe-ready`, 방 콘텐츠 `room-ready-*`, 그래프 방문 상태 `dungeon-room-ready-<node-id>-<room-type>-<state>`로 각 런타임 준비를 동기화한 뒤 `move`, `move-direction-*`, `move-motion-direction-*`, `move-step-direction-*`, `player-cell-x-*-z-*`, `chaser-cell-x-*-z-*`, `boss-cell-x-*-z-*`, `boss-move-target-x-*-z-*`, `boss-moved`, `boss-move-blocked`, 적 상태·이동, 폭탄 설치·폭발·피해·클리어, `combat-reward-tokens-<count>`, `room-reward-tokens-<count>`, `secret-wall-revealed-room-<id>-direction-<direction>`, `secret-reward-collected-<count>`, `dungeon-transition-started`, `dungeon-room-committed`, `bomb-reward-selected-<definition-id>`, `bomb-exploded-definition-<definition-id>`, `line-bomb-placed-<direction>`, `line-bomb-exploded-<direction>`, `swap-bomb`, `pause-entered`, `pause-resumed`, `pause-resume`, `audio-unlocked`를 제공한다. 장갑병은 추가로 `armored-panic-telegraph-<direction>-distance-<count>`, `armored-panic-run-moved`, `armored-panic-recover`, `armored-chase`를, 자폭병은 `self-destruct-cell-x-<x>-z-<z>`, `self-destruct-moved`, `self-destruct-warning-chase`, `self-destruct-armed`, `self-destruct-telegraph`, `self-destruct-detonated`, `self-destruct-died`를 확정 Core 전이에서 기록한다. Gates 구간은 graph 경계 입장 뒤 오른쪽 아래 `(3,-3)`에서 자폭병을 우회로 `(3,-1)`까지 끌어내고, 플레이어가 `(-1,-2)`로 가로질러 자폭병이 아래 유도 셀 `(0,-2)`에 도착한 뒤 점화하는지, 안전 셀 `(-1,-3)`으로 이탈하는지, `bomb-exploded-definition-prototype-self-destruct-blast`와 진입 뒤 증가한 `destructible-wall-destroyed`가 함께 발생하는지 요구해 적 사망 marker만으로 통과하지 않는다. pause marker는 버튼 callback이 아니라 `PrototypeGameSession.PauseStateChanged`의 확정 전이만 기록한다. `swap-bomb`은 입력 수신이고 `active-bomb-slot-1`은 쿨타임을 통과한 Core 교체다. 명령 probe는 `PlayerCommand.Move`, motion probe는 frame 연속 위치 변화, step·cell probe는 정수 셀 경계 전이와 현재 좌표를 기록한다. 빠른 단타 검사는 key down 뒤 motion을 기다렸다가 release하는 방식이 아니라 down/up을 먼저 연속 전송한 뒤 motion 한 frame과 50ms 정지 안정성을 확인한다. 기본 smoke는 Secret 왕복을 포함한 실제 graph 전환과 각 commit, 주 경로 전투 3개 클리어를 관측한다. 첫 전투의 `active → room-cleared`, 금 간 출구의 실제 폭발 공개, Secret `safe`·cache, 보상 선택, 재입장의 `cleared`와 일정 시간 적 사건 0, 후속 전투 2개의 `active → room-cleared`, 보스 전실 `safe`, 보스 `active`와 목적지 예고·확정 이동·선행 설치 적중을 서로 다른 marker로 구분한다. 갑옷 전용 smoke는 별도 시작 씬에서 첫 폭발의 `armored-broken → armored-panic-telegraph-east-distance-3`, 실제 질주·회복·추격, 다른 위치의 두 번째 설치 뒤 `armored-died → enemy-died` 순서를 확인한다. 필수 추격자가 남으므로 전용 smoke의 책임에는 `room-cleared`가 없고 다중 적 클리어는 PlayMode와 기본 던전 smoke가 확인한다. 방향성 직선 전용 smoke는 설치 직후 다른 이동 명령을 보낸 전후의 방향 marker를 순서대로 확인한다. 피해·이동·설치·폭발·상태·사망·클리어·보상 표식은 실제 Core 상태 전이만 기록한다. Core 연속 좌표와 Transform 일치는 PlayMode 통합 테스트가 확인한다. `audio-unlocked` marker는 오디오 출력 자체를 증명하지 않으므로 오디오 연결 뒤 수동 항목을 별도로 통과해야 한다.
