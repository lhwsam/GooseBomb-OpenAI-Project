...
- 보스 격파 뒤 `FLOOR CLEARED` 결과 UI가 표시되고 전투가 멈춘다. 완료 화면을 캡처한 뒤 `R`을 눌러 페이지 reload 없이 새 seed-0 시작방이 준비되고 보상·방 클리어 상태가 초기화됐는지 확인한다.
- 사용자 입력 뒤 오디오 재생.
...
완료·재시작 구간의 추가 marker는 `run-completed`, `run-restart-requested`, `dungeon-run-restarted`이며, 재시작 뒤 두 번째 `dungeon-room-ready-1-start-safe`까지 관찰해야 통과한다.
