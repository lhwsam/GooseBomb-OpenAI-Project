...
- 코드 소유: 결과 상태는 Core `DungeonRunState`, 방 사건 연결은 `PrototypeDungeonRoomBinder`, 씬 수명과 초기화는 `PrototypeDungeonRunHost`, 보스 완료·플레이어 사망 표현 gate는 `PrototypeBossClearPresenter`·`PrototypePlayerDeathPresenter`, 결과 UI와 입력 소비는 `PrototypeRunCompletionPresenter`
...
6. `PrototypePlayerDeathPresenter`는 어느 방에서든 치명 피해가 확정되면 방 session을 즉시 비활성화하고 기존 폭발 흔들림을 끝낸다. 전역 `Time.timeScale`이나 Core 시계를 바꾸지 않고 플레이어 Animator만 unscaled 0.75배속으로 재생하며, 1초 카메라 focus·사망 clip 완주·0.2초 pose 유지·curtain close가 끝날 때까지 실패 결과 화면 공개 gate를 닫는다. 보스 격파 burst나 추가 사망 VFX는 만들지 않는다.
...
- 완료 결과 UI는 격파 연출 gate가 열린 뒤에만 나타난다. 실패 결과 UI는 보스 격파 gate가 아니라 플레이어 사망 애니메이션·curtain gate가 열린 뒤에만 나타난다.
...
- 11개 던전·TestSandbox 씬 모두 Systems 오브젝트에 `PrototypeRunCompletionPresenter`와 `PrototypePlayerDeathPresenter`를 한 개씩 가지며 같은 session·player controller·player Animator·MainCamera·camera shake와 공유 curtain 프리팹을 명시적으로 참조한다.
- `DungeonBoss`의 presenter는 같은 scene의 `PrototypeBossClearPresenter`를 명시적으로 참조하고 gate 뒤 완료 UI를 표시한다. 독립 `BossBattlePlaytest`에는 런 결과 presenter를 두지 않는다.
...
- PlayMode는 session 위임, 치명 피해 snapshot 보존, source와 고정 적 ID의 사망 원인 매핑, 실제 사망 애니메이션 완주 전 결과 UI 차단과 카메라·Animator 복원, 완료·실패 상태의 새 run 재시작과 terminal host 제거→로비→새 run 수명을 검증한다.
- Development WebGL smoke는 실제 보스 격파 뒤 결과 UI로 로비에 복귀해 다시 시작한다. 이어 안전방 자기 폭발 5회로 `player-died → player-death-presentation-started → player-death-presentation-completed → run-failed → run-failed-cause-bomb-explosion`을 관찰하고, 사망 연출 완료 전 결과 UI가 나타나지 않는지 확인한다. 이후 `CAUSE: BOMB EXPLOSION` 실패 화면을 캡처하고 `R`로 새 시작방까지 확인한다.
