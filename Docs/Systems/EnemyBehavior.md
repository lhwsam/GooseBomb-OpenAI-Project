...
프로토타입에서는 격자 기반의 단순하고 결정적인 BFS 거리장을 사용한다. 거리장은 매 frame이 아니라 첫 판단, 방향 유지 종료, 즉시 차단, 계획 거리 증가 시점에만 다시 만든다. 목표 셀과 추격자의 현재 셀은 탐색 가능하고, 그 밖의 actor·폭탄 점유와 비바닥은 차단한다. 경로가 없으면 임의 배회하지 않고 다음 cadence까지 기다린다. 탐색용 `Dictionary`·`Queue`는 simulation이 재사용하며 이 규칙은 플레이어 미래 위치나 폭발 위험을 계산하지 않는다. AI Navigation 패키지는 Core 경로 판정의 원본이 아니다.
