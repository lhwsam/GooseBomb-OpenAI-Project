...
| 자폭병 | 홀로그램 점멸과 애니메이션으로 경고하며 추적하다 인접 시 멈춰 적·문·퇴로를 함께 바꾸는 적 폭발 | Chase, WarningChase, Telegraph, Detonated |
...
- `PrototypeChaserPresenter`는 Core가 계산한 연속 위치와 Idle/Run·Attack·Die 애니메이션을 표시한 뒤 사망 표현 시간이 끝나면 모델을 비활성화한다. 상태와 사망에 따라 본체 material 색을 덮어쓰지 않으며, 실제 피해 때만 선택적 Scanline 홀로그램을 빠르게 두 번 표시한다. 접촉 가능 시각은 Transform을 읽지 않고 Core의 판정 칸과 도착 시각으로 계산한다.
...
- `PrototypeChargerPresenter`는 고정 최대 거리의 모든 논리 셀에 collider 없는 얇은 예고 placeholder를 풀링해 표시하고 `Charge` 시작 때 회수한다. 로컬 Holograms 설정이 있으면 이 셀들을 공용 폭탄 범위와 같은 주황 Grid material·알파·emission으로 표시하며, 설정이 없으면 기존 저작 material을 유지한다. 획득 이동과 돌진 이동의 연속 위치는 각각의 Core 이동 시간으로 계산된다.
...
- `PrototypeArmoredPresenter`는 저작 material 색을 유지하고 실제 피해 때만 선택적 Scanline 홀로그램을 빠르게 두 번 표시하며, scale 축소로 파괴 상태를 보조 표현한다. collider 없는 얇은 셀 placeholder를 최대 3개 풀링해 고정 panic 경로를 표시하고, 로컬 Holograms 설정이 있으면 공용 폭탄 범위와 같은 주황 Grid material·알파·emission을 사용한다. Run 시작 또는 사망 때 셀을 회수하며, 수비·panic·추격 각각의 논리 cadence로 확정 이동을 보간한다.
...
- `WarningChase`는 로컬 Holograms 설정이 있으면 Scanline 홀로그램을 0.14초 간격으로 점멸하고, `Telegraph`에서는 이동을 멈춰 권위 셀에 고정한 채 0.065초 간격으로 더 빠르게 점멸하며 Telegraph 애니메이션과 실제 범위 셀을 함께 표시한다. 기존 scale pulse는 제거해 두 상태 모두 저작 scale을 유지한다. package가 없으면 홀로그램만 생략하고 이동·애니메이션·범위 셀은 그대로 유지한다.
...
- 적 폭탄 자체는 일반 `BombPlaced` 플레이어 사건을 만들지 않지만 폭발은 공용 `BombExploded` 경로를 재사용한다. 자폭병의 `prototype-self-destruct-blast`는 실제 `AffectedCells`가 확정된 뒤 플레이어 십자 폭발과 같은 중심·직선 VFX 풀을 사용하고, 로컬 VFX 연결이 없으면 기존 셀 placeholder로 대체한다.
...
- Telegraph 종료 뒤 서로 다른 세 목표로 0.45초 표현 비행을 동시에 시작한다. Telegraph와 비행 중에는 착지 셀과 폭발 범위를 표시하지 않는다. 이때는 아직 격자를 점유하는 폭탄이 아니며, 각 착탄 순간 같은 `BombSimulation`에 `ActorId(7)` 소유 폭탄으로 설치해 fuse·벽 차단·0.15초 연쇄 지연을 기존 폭탄과 공유한다.
...
- `PrototypeThrowerPresenter`는 collider와 Rigidbody가 없는 `ThrowerPig` 프리팹을 Core 연속 위치에 표시한다. Track 이동은 Idle/Walk, Telegraph 진입은 Throw와 본체 pulse, Recover는 Idle, 사망은 terminal Die 표현으로 변환하고 Root Motion은 사용하지 않는다. 잠긴 목표 3개의 셀 시각은 만들지 않는다. `PrototypeBombPresenter`는 세 포물선 비행을 각각 풀링 표현한 뒤 성공 착탄 시 대응 폭탄 표현으로 넘기고, 그때 현재 논리 격자에서 계산한 Core 폭발 미리보기 범위를 표시한다. 겹치는 셀은 중복하지 않고 실제 폭발한 `BombId`의 범위만 제거한다.
...
- 추격자·돌진형·갑옷 적·자폭병·투척병·보스 본체는 상태·phase·사망에 따라 material 색을 덮어쓰지 않는다. 적용된 피해에만 선택적 Scanline 홀로그램을 빠르게 두 번 표시하고 종료 시 원본 shared material과 속성을 복원한다. 공격 범위, 돌진 경로, 이동 목적지 같은 별도 위험 셀의 색은 공간 예고로 유지한다.
...
- PlayMode의 자폭병 경고 추적 상태와 저작 scale 유지·선택적 홀로그램 점멸, 이동 중 플레이어 폭발 trigger 시 권위 셀 고정, 기존 폭탄 스케줄러의 실제 자폭과 파괴벽 변경·단일 사망 사건.
- Development WebGL의 `self-destruct-cell-x-<x>-z-<z>`와 `self-destruct-warning-chase` marker로 실제 확정 셀·경고 진입을 관측한다. 이 marker는 자동 경로 동기화용이며 홀로그램 점멸·애니메이션의 가독성이나 행동의 재미를 통과 판정하지 않는다.
