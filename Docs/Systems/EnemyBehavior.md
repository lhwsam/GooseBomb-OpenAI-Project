...
- `ChargerEnemyDefinition`은 안정적인 `EnemyDefinitionId`, 내구도, 충돌 피해, 예고 시간, 한 셀 돌진 cadence와 회복 시간을 소유한다. TestSandbox의 `prototype-charger` 값은 내구도 1, 충돌 피해 1, 예고 0.75초, 돌진 8 cells/s, 회복 0.75초이며 플레이테스트 전까지 `Proposed`다.
- `ChargerEnemySimulation`은 `ActorId(3)`으로 추격자와 같은 `GridState`를 점유한다. 플레이어와 같은 행 또는 열이고 중간 셀이 모두 비어 있을 때만 `Track → Telegraph`가 되며 예고 시작 순간 방향을 잠근다.
- 예고 중 플레이어가 이동해도 방향을 바꾸거나 취소하지 않는다. 예고 종료 뒤 `Charge`에서 고정 방향으로 한 셀씩 이동하고, 플레이어가 다음 셀에 있으면 겹치지 않은 채 단일 충돌 피해 후보를 만든 뒤 `Recover`가 된다.
- 고정 벽, 파괴 가능 벽, 폭탄 또는 다른 actor가 다음 셀을 막으면 피해 없이 `Recover`가 된다. 회복 종료 뒤에만 다시 정렬을 탐지한다.
