...
- `Interactable`은 회복 pickup과 비밀 cache처럼 인접 입력으로 사용하는 월드 오브젝트다. 활성 동안 actor 이동·이동 예약·폭탄 설치를 막고, 성공적으로 소비된 뒤 점유를 제거해 셀을 연다.
...
- 유지 방향은 각 10ms simulation step에서 주입 시계의 경과 시간 × 기본 5 cells/s만큼 연속 위치를 진행한다. `Move(None)` 동안 위치는 변하지 않고 정지 중 시간도 다음 입력에 누적하지 않는다.
- 방향 변경은 별도 셀 cadence나 pending queue 없이 다음 10ms 관찰 step의 이동 축에 적용한다. 상하좌우만 허용하므로 한 step의 변위는 한 축에만 생긴다.
- 플레이어는 접근 중인 다음 셀만 예약한다. 셀 경계를 통과할 때 `GridState.TryCommitReservedActorMove`로 정수 점유를 전이하고 예약을 완료한 뒤 `PlayerMovementStep`을 발행한다. 큰 frame은 열린 각 셀을 순서대로 검사해 장애물을 건너뛰지 않는다.
...
- 이 경계 규칙은 위치를 셀로 해석하는 좌표 계약이다. 플레이어 연속 이동은 경계를 실제 통과할 때만 예약을 commit해 정수 점유를 전환한다.

## 핵심 불변식

- 하나의 정적 벽 셀은 동시에 바닥이나 다른 벽 종류가 될 수 없다.
- 폭탄 설치 가능 여부는 논리 점유로 판정한다.
- 상호작용 오브젝트는 동일 셀의 actor·bomb·다른 interactable 및 actor 예약과 공존하지 않는다. 성공·이미 소비 상태에서만 제거하며, 최대 체력 회복 실패처럼 소비하지 않은 결과는 점유를 유지한다.
...
- 플레이어 충돌 반경. 공간 기반 코너 보정과 중심선 스냅은 현재 이동 계약에 추가하지 않는다.
...
- 10ms 고정 simulation step 기반 연속 진행, 해제 즉시 정지·시간 비누적, 빠른 방향 반복, 다중 셀 경계 순회와 막힌 셀 중심 제한.
...
- 추격자가 플레이어와 공유하는 논리 격자에서 예약 이동하고 presenter가 Core 연속 위치를 표시함.
...
## Actor 이동 정책 분리와 회귀 복구

- 모든 Actor는 한 관찰 구간에 하나의 cardinal 축으로만 이동한다. X와 Z를 동시에 바꾸는 대각선 이동은 허용하지 않는다.
- 적 Actor는 한 번에 인접한 한 칸을 예약하고, 시작된 칸 이동을 목적지 중심까지 완료한다.
- 플레이어는 위의 `구현된 플레이어 이동 계약`과 [프레임 반응형 플레이어 이동](../Development/ContinuousPlayerMovementSlice.md)을 따른다. 입력 해제나 변경 뒤 한 칸 중심까지 강제로 완료하지 않는다.
- 적은 칸 이동을 시작할 때 목적지를 예약하고, 플레이어는 현재 진행 방향으로 접근하는 다음 셀만 예약한다. 예약된 칸에는 다른 Actor가 이동하거나 새 폭탄이 설치될 수 없다.
- 이동 진행률이 50% 미만이면 출발 칸, 50% 이상이면 목적지 칸을 현재 판정 칸으로 사용한다.
- 폭탄 설치, 폭발 피해, 접촉 판정은 Transform이 아니라 현재 판정 칸을 사용한다.
- 적은 이동이 100% 완료되기 전 다음 칸 이동을 시작하지 않는다. 승인된 플레이어 연속 이동은 다음 관찰 step에 바뀐 cardinal 입력을 적용하되 예약·점유를 원자적으로 정리해야 한다.
- 세션은 플레이어·적·폭탄·공격·피해를 10ms 고정 스텝마다 함께 진행한다. 프레임에 남은 10ms 미만 시간은 다음 프레임으로 넘기며, 큰 프레임 delta에서도 한 Actor만 여러 칸을 선처리하지 않는다.
- Chaser·Charger·SelfDestruct·Thrower는 committed tile 계약을 사용한다. Armored와 보스 이동의 통합은 후속 범위다.
