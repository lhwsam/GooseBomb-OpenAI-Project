...
- 최초 방향 입력은 즉시 한 step을 시도한다. 누르고 있으면 기본 0.2초 간격으로 반복한다. 입력 의도는 두 수직 키가 겹친 순간 새 방향으로 즉시 바뀌지만, 논리 이동과 표현의 실제 꺾임은 현재 셀 이동이 끝나는 다음 예약 step에서 적용한다.
...
- 주입 시계 기반 최초/반복 cadence, 다음 셀 경계 방향 변경, 정지·재개 우회 방지.
