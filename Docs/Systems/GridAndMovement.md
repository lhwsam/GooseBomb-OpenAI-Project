...
- `9f4cf04`가 적의 완료 계약을 플레이어에도 적용해 키 해제·방향 변경을 최대 0.2초 늦춘 회귀는 플레이어 전용 연속 이동으로 복구했다. `10 cells/s + 0.2초 반복 대기` 칸 이동 후보는 사람 플레이에서 `Rejected`되어 제거했으며, 현재 계약은 기본 `5 cells/s` 연속 이동이다. 판정과 검증 근거는 [이동 응답성 회귀 기록](../Development/PlayerMovementResponsivenessRegression.md)이 소유한다.
