...
- `PrototypeBombPresenter`는 정의 ID별 설치 폭탄과 영향 셀 placeholder 풀을 사용하고, 직선 폭탄의 비대칭 설치체를 확정된 방향으로 회전한다. 또한 모든 `BombActivated`를 같은 Core 폭발 미리보기 API로 조회해 fuse 동안 하나의 공용 위험 셀 풀로 표시한다. 여러 활성 폭탄의 겹친 셀은 한 번만 표시하고 `BombExploded` 때 해당 `BombId`의 기여만 제거한다. 로컬 Holograms 설정이 있으면 이 셀 renderer의 shared material을 Grid 홀로그램으로 교체하고 `MaterialPropertyBlock`으로 주황색 알파 `0.14`, tint 알파 `0.065`, emission `3.5`를 적용해 기본 예제보다 낮은 밝기로 표시한다. 설정이 없으면 기존 위험 셀 material을 유지하며, 어느 경로도 셀마다 material 인스턴스를 만들지 않는다. 이 설치 후 범위는 투척 전 목표 셀과 보스 패턴 선행 예고와 분리한다. 공개 플레이어 폭탄 prefab 3종은 빈 `SparksEffect` 앵커를 fallback으로 유지하므로 로컬 VFX 패키지가 없는 clone에서도 Missing 참조 없이 동작한다. 권한 있는 작업자는 `Bomb Swap/Local Setup/Connect Licensed VFX`로 현재 패키지의 준비 파티클 prefab을 앵커 아래 `Particle` 자식으로 연결할 수 있다. 공개 변경을 준비할 때는 `Bomb Swap/Local Setup/Reset Player Bomb VFX to Public Fallback`으로 로컬 자식을 제거한다. 로컬 VFX가 있으면 presenter와 연결 메뉴가 앵커를 설정 asset의 심지 위치·회전으로 정규화하고, 하위 ParticleSystem을 Local simulation·Collision 비활성으로 강제해 폭탄 설치 방향이나 주변 Collider가 불꽃 궤적을 바꾸지 않게 한다. 설치 때 앵커를 Animator와 함께 활성화하고 폭발 때 Animator를 비활성화한다.
...
- 자폭병의 `prototype-self-destruct-blast`, 투척병의 `prototype-thrower-blocker`와 보스의 `prototype-boss-throw`·`prototype-boss-chain`도 최종 폭발 시 같은 중심·십자 직선 VFX 풀을 사용한다. 자폭병 점화와 자기 사망, 비행·착지·fuse·연쇄 규칙은 바꾸지 않고, 각 폭탄의 실제 Core `AffectedCells`가 확정된 뒤 표현만 교체한다.
