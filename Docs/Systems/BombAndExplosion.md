...
- `PrototypeBombPresenter`는 정의 ID별 설치 폭탄과 영향 셀 placeholder 풀을 사용하고, 직선 폭탄의 비대칭 설치체를 확정된 방향으로 회전한다. 로컬 VFX 연결 메뉴는 플레이어 폭탄 3종의 `SparksEffect` 앵커 아래 기존 정상·Missing 자식을 제거하고 현재 패키지의 준비 파티클 prefab을 `Particle` 자식으로 다시 저장한다. 앵커의 저작 위치·회전을 유지하고 설치 때 Animator와 함께 활성화하며 폭발 때 Animator를 비활성화한다. 폭탄 prefab의 참조는 커밋할 수 있고 원본 VFX는 제외한다. 패키지가 없는 clone의 Missing 참조는 패키지 import 후 원클릭 메뉴가 복구한다.
