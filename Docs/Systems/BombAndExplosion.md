...
- `PrototypeGameSession`은 이동·폭탄·두 슬롯이 공유하는 하나의 논리 격자·시계를 소유하고 `PlaceBomb` 명령을 활성 슬롯, 현재 플레이어 셀과 Core의 마지막 바라보기 방향에 적용한다. 플레이어·자폭병·투척병·보스가 실제 `BombSimulation`에 설치를 확정하면 소유자별 사건과 별도로 공통 `BombActivated` snapshot을 발행한다.
...
- `PrototypeBombPresenter`는 정의 ID별 설치 폭탄과 영향 셀 placeholder 풀을 사용하고, 직선 폭탄의 비대칭 설치체를 확정된 방향으로 회전한다. 또한 모든 `BombActivated`를 같은 Core 폭발 미리보기 API로 조회해 fuse 동안 하나의 공용 위험 셀 풀로 표시한다. 여러 활성 폭탄의 겹친 셀은 한 번만 표시하고 `BombExploded` 때 해당 `BombId`의 기여만 제거한다. 로컬 Holograms 설정이 있으면 이 셀 renderer의 shared material을 Grid 홀로그램으로 교체하고 `MaterialPropertyBlock`으로 주황색 알파 `0.14`, tint 알파 `0.065`, emission `3.5`를 적용한다. 설정이 없으면 기존 위험 셀 material을 유지하며, 어느 경로도 셀마다 material 인스턴스를 만들지 않는다. 플레이어 3종과 Boss·Enemy 폭탄 prefab은 공통으로 루트 Animator, `visual`, 빈 `SparksEffect` 앵커를 유지하므로 로컬 VFX 패키지가 없는 clone에서도 Missing 참조 없이 동작한다. `Connect Licensed VFX`와 `Reset Bomb VFX to Public Fallback`은 다섯 폭탄을 함께 처리하되 플레이어 폭탄에만 로컬 설정의 심지 위치·회전을 적용하고 Boss·Enemy 폭탄의 저작 앵커 Transform은 보존한다. 모든 하위 ParticleSystem은 Local simulation·Collision 비활성으로 정규화한다. 설치 때 앵커를 Animator와 함께 활성화하고 폭발 때 Animator를 비활성화한다.
- 같은 presenter는 모든 소유자의 공통 `BombActivated`에서 fuse loop 3D SFX를 논리 셀에 시작하고 `BombExploded`에서 해당 `BombId`의 fuse voice를 정지한 뒤 두 폭발 clip 중 하나를 재생한다. 탑다운 카메라의 Listener 높이와 방 크기를 포함하도록 선형 감쇠의 최소·최대 거리를 각각 18·30으로 두고, 원본 레벨이 낮은 fuse는 source volume 1, 폭발은 0.9를 사용한다. voice는 SFX Mixer로 route하고 풀링하며 pause·비활성화·보스 클리어 정리에서 누수 없이 정지한다. 소유자별 폭탄 규칙이나 Core fuse 시각은 오디오에 의존하지 않는다.
...
- 플레이어의 `Cross` 폭발은 로컬 VFX가 연결되어 있으면 폭발 원점보다 월드 Y 0.5 높은 위치의 중심에 `vfx_Explosion`, 실제 Core 폭발 셀이 존재하는 각 방향에 `vfx_Bomb_Straight` 하나를 재생한다. 방향 파티클은 로컬 Z축을 북·동·남·서로 회전하고, 연속 영향 셀 수에 `0.25`를 곱해 `Flames_F`의 speed modifier로 사용한다. 현재 플레이어 폭탄은 1~5셀을 `0.25`~`1.25`로 변환한다. 벽에 바로 막혀 영향 셀이 0개인 방향은 재생하지 않는다. 이 VFX는 폭발 정의의 기존 0.25초 셀 표시와 별개로 최소 1초 동안 유지해 저작 파티클을 조기에 자르지 않는다. VFX가 연결되지 않았거나 지원 대상이 아닌 소유자·정의의 폭발이면 기존 영향 셀 placeholder 표현을 유지한다. 모든 폭발 표현과 파티클 계층 참조는 풀링·캐시되며 표시 시간이 끝나면 반환한다. 풀을 초과하면 규칙을 누락하지 않고 표현 인스턴스만 확장한다.
...
- 자폭병의 `prototype-self-destruct-blast`는 기존 설치체를 유지하고, 투척병의 `prototype-thrower-blocker`는 `EnemyBomb.prefab`, 보스의 `prototype-boss-throw`·`prototype-boss-chain`은 공용 `BossBomb.prefab` 설치체를 사용한다. 모두 최종 폭발 시 같은 중심·십자 직선 VFX 풀을 사용한다. 자폭병 점화와 자기 사망, 비행·착지·fuse·연쇄 규칙은 바꾸지 않고, 각 폭탄의 실제 Core `AffectedCells`가 확정된 뒤 표현만 교체한다.
...
- 자폭병은 Telegraph 시작 시 `ActorId(6)` 소유의 `prototype-self-destruct-blast`를 자기 셀에 직접 설치한다. 이 적 폭탄은 `BombSimulation`의 활성 폭탄과 연쇄 스케줄러 및 공용 설치 후 범위 표시에 포함되지만 플레이어 loadout·설치 쿨타임·`BombPlaced` 입력 성공 사건에는 포함되지 않는다.
...
- 투척병은 Telegraph에서 서로 다른 목표 셀 3개를 잠근 뒤 세 방향으로 0.45초 표현 비행을 동시에 시작한다. Telegraph와 비행 중에는 착지 셀, 폭탄 점유와 폭발 범위 표시가 없다. 각 착탄 순간에만 `ActorId(7)` 소유 `prototype-thrower-blocker`를 같은 `BombSimulation`에 설치한다. 이미 폭탄이 있는 셀의 발만 조용히 실패하며 다른 셀로 재조준하지 않는다.
- 성공 착탄한 세 투척 폭탄은 플레이어 폭탄과 같은 fuse·벽 차단·파괴벽·연쇄 스케줄러를 각각 사용한다. 착탄 뒤에는 소유자 전용 프레젠터가 아니라 공용 폭탄 범위 표시가 현재 Core 미리보기를 fuse 동안 보여준다. 투척병은 자기 소유 폭발만 무시하고 다른 소유자의 폭발에는 정상 피해를 받는다.
- 보스는 Telegraph 시작 시 잠근 퇴로 anchor에 `ActorId(5)` 소유 throw 폭탄을 설치하고, 2페이즈에는 그 안쪽 cardinal 셀에 chain 폭탄도 설치한다. throw footprint가 chain 셀을 포함하므로 전역 0.15초 연쇄 지연을 그대로 사용한다. 투척 목적지는 비행 전 표시하지 않고 실제 착탄한 폭탄의 fuse 범위만 공용 폭탄 범위 표시로 전환한다. parity처럼 폭탄과 분리된 공간 공격 패턴은 자체 선행 예고를 유지한다. 보스 폭탄은 플레이어·일반 적·벽에 정상 반응하지만 소유자인 보스는 자기 폭발 피해를 무시한다.
...
- 직선 앞쪽 범위 5(원점 포함 6셀)의 네 cardinal 셀 순서, 옆·뒤 제외, 설치 방향 고정과 방향 누락 거부.
