...
- `PrototypeBombPresenter`는 정의 ID별 설치 폭탄과 영향 셀 placeholder 풀을 사용하고, 직선 폭탄의 비대칭 설치체를 확정된 방향으로 회전한다. 플레이어 3종과 Boss·Enemy 폭탄 prefab은 공통으로 루트 Animator, `visual`, 빈 `SparksEffect` 앵커를 유지하므로 로컬 VFX 패키지가 없는 clone에서도 Missing 참조 없이 동작한다. 권한 있는 작업자는 `Bomb Swap/Local Setup/Connect Licensed VFX`로 현재 패키지의 준비 파티클 prefab을 다섯 앵커 아래 `Particle` 자식으로 연결할 수 있다. 공개 변경을 준비할 때는 `Bomb Swap/Local Setup/Reset Bomb VFX to Public Fallback`으로 로컬 자식을 제거하며, 두 메뉴 모두 앵커와 모델의 저작 Transform을 보존한다. 설치 때 앵커를 Animator와 함께 활성화하고 폭발 때 Animator를 비활성화한다.
...
- 투척병의 `prototype-thrower-blocker`는 `EnemyBomb.prefab`, 보스의 `prototype-boss-throw`·`prototype-boss-chain`은 공용 `BossBomb.prefab` 설치체를 사용하고 최종 폭발 시 같은 중심·십자 직선 VFX 풀을 사용한다. 비행·착지·fuse·연쇄 규칙은 바꾸지 않고, 각 폭탄의 실제 Core `AffectedCells`가 확정된 뒤 표현만 교체한다.
...
- 성공 착탄한 세 투척 폭탄은 플레이어 폭탄과 같은 fuse·벽 차단·파괴벽·연쇄 스케줄러를 각각 사용한다. 착탄한 폭탄별 현재 Core 폭발 미리보기 범위를 fuse 동안 표시하고, 겹친 셀은 하나로 보이되 실제 폭발 때 해당 `BombId`의 범위만 제거한다. 투척병은 자기 소유 폭발만 무시하고 다른 소유자의 폭발에는 정상 피해를 받는다.
