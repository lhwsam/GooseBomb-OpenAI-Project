...
- `PrototypeBombPresenter`는 정의 ID별 설치 폭탄과 영향 셀 placeholder 풀을 사용하고, 직선 폭탄의 비대칭 설치체를 확정된 방향으로 회전한다. 공개 플레이어 폭탄 prefab 3종은 빈 `SparksEffect` 앵커를 fallback으로 유지한다. 권한 있는 작업자가 `Bomb Swap/Local Setup/Connect Licensed VFX`를 실행하면 앵커 아래 기존 정상·Missing 자식을 제거하고 현재 패키지의 준비 파티클 prefab을 `Particle` 자식으로 다시 저장한다. 공개 변경 전에는 `Bomb Swap/Local Setup/Reset Player Bomb VFX to Public Fallback`으로 로컬 자식을 제거한다. 두 메뉴 모두 앵커의 저작 위치·회전을 유지한다. 설치 때 앵커를 Animator와 함께 활성화하고 폭발 때 Animator를 비활성화한다. 폭탄 prefab의 참조는 커밋할 수 있고 원본 VFX는 제외한다.
