...
- `DungeonGenerator.Generate(seed, definition)`은 모든 `int` seed를 받고 `prototype-tree-v2` 결과를 만든다. 시스템 시간, 호출 횟수, `System.Random` 또는 `UnityEngine.Random`을 읽지 않는다.
...
- 현재 `RoomType`은 기존 직렬화 값 `Combat = 0`, `Start = 1`, `BombReward = 2`, `BossAntechamber = 3`, `Boss = 4`를 보존하고 끝에 `Recovery = 5`를 추가한다. 다섯 특수방은 실제 씬으로 저작됐으며 보상방은 첫 폭탄 선택을, 회복방은 일회성 `+2` 회복을 제공한다.
...
- `PrototypeDungeonSpecialRoomCatalogAsset`이 시작·폭탄 보상·보스 전실·회복·보스 타입의 서로 다른 씬 이름을 제공하며, 실제 catalog asset과 다섯 특수방 씬을 통해 run session이 모든 노드를 씬으로 해석한다.
...
선택 전투 가지의 접속 지점과 4/5 전투방 수, 정수 방 좌표는 seed로 달라진다. 선택 전투 가지는 폭탄 보상 이후에만 도달하며 보스 주 경로에는 포함되지 않는다. Recovery는 seed와 무관하게 보스 주 경로의 마지막 일반 전투방에 붙는 단일 막다른 leaf이며, 보스 전실로 가기 전 우회 선택을 만든다.
...
6. 노드별 고정 방향 후보 순서로 정수 XZ 좌표를 유한 backtracking 배치한다. 연결되지 않은 방의 cardinal 인접은 거부해 암시적 문과 루프를 막는다.
7. 연결 수, 좌표, 필수 노드, 첫 보상, 보스 경로, 선택 전투 가지와 Recovery leaf를 `DungeonGraph` 생성 시 다시 검증한다.
8. Core `DungeonRunState`가 인접 이동, 최초 방문, 전투 잠금·클리어와 양방향 재방문을 처리한다.
9. 전투 노드별 활성 출구와 호환되는 수제 방 정의·회전을 결정적으로 배정한다.
...
- 같은 정의·동일 seed 재현과 `prototype-tree-v2` seed 0 golden snapshot.
