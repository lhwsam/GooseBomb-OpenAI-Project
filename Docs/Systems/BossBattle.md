...
보스전은 별도 물리나 탄막 규칙이 아니라 정수 XZ 격자, 기존 폭탄·벽 차단·연쇄·자폭병을 결합한 종합 시험이다. 플레이어는 무한 추격을 피하는 대신 보스를 유도하고, 순차 투척과 parity 파동 중 자신의 두 폭탄을 선행 설치한 뒤 짧은 과열에 회수한다.
...
- 현재 시퀀스와 과열이 끝나는 안전 경계에서만 phase를 바꾼다.
...
- `Overheat`의 Recovery만 플레이어 폭탄 취약 상태다.
- 서로 다른 `BombId` 하나당 피해 1, 한 과열의 누적 상한은 2다.
- 같은 `BombId`, 상한 뒤 폭발, 과열 밖 폭발, 사망 뒤 폭발은 상태를 구분해 거부한다.
- One/Two/LastStand 실제 과열 시간은 2.0/1.5/2.25초다. 모두 첫 사람 테스트 전 `Proposed`다.
- 과열 진입 시 누적을 0으로 초기화하며 phase가 바뀌어도 이전 과열의 상한을 넘겨 적용하지 않는다.
...
- 체력 10, Two 임계 7, LastStand 임계 2, 패턴 피해 1, 과열 피해 상한 2
...
- 플레이어 폭탄은 과열당 최대 2, 자폭병은 과열 밖에서도 최대 한 번 피해를 준다.
- phase 전환은 현재 시퀀스 중간에 일어나지 않고 LastStand를 생략하지 않는다.
- 보스 사망과 방 클리어는 한 번만 발생한다.

## 검증

- EditMode: 정의·3 phase, 추격·돌진, 순차 투척, parity 행, 과열 상한, 지연 전환, 소환 셀 잠금·해결 대기, 자폭병 피해, LastStand 단일 실행, 사망·시계·저작 앵커 검증.
- PlayMode: 세션 전이, 포물선 flight→landing→fuse, 예약 셀, 자폭병 동적 생성·강제 점화, presenter pause, HUD phase, 실제 던전 보스 씬 회귀.
- Content: 보스 정의, 폭탄/prefab, 6 투척·3 소환 앵커, 전용 플레이테스트 씬과 기존 Build Settings 유지.
- 사람: 한 과열 두 폭탄 시도, 자폭병 보스 유도, parity 안전 칸 재사용, phase 차이·최후 발악 공정성, 60~150초 목표 시간.
- WebGL: 11씬 Development 빌드와 Edge keyboard `46/46`, 가상 표준 Gamepad `14/14`가 통과했다. keyboard 경로는 1페이즈 2회, 일회성 자폭병, 2페이즈 2회, LastStand, 과열당 서로 다른 폭탄 2개와 보스 처치를 검증하며 Console/page error는 0이다.
