...
여러 적 spawn 후보와 제약, 보상·전환 anchor는 아직 구현하지 않았으며 방 그래프 작업에서 확장한다. 첫 구현은 독립 방 prefab 대신 이 데이터와 `TestSandbox` 씬 표현을 연결한다. 향후 prefab으로 분리해도 논리 셀 데이터가 권위 원본이라는 경계는 유지한다.
...
| 3 | `prototype-combat-pillars` / `PrototypeCombatPillars.asset` | 플레이어 `(-3, -2)`, 추격자 `(3, 2)` | 엇갈린 고정 기둥 5개, 중앙 파괴 벽 `(0,0)`: 파괴 전 엄폐·파괴 후 공간 확장 | `TestSandboxPillars.unity` / 없음 |
