...
첫 방은 북·남, 두 번째는 북·남, 세 번째는 서·동, 네 번째는 북·남 경계 출구를 갖는다. 각 방은 서로 다른 첫 cardinal 이동을 쓰는 퇴로 anchor 두 개와 닫힌 cardinal 유도 순환 경로를 소유한다. 출구는 아직 실제 문이나 런 그래프로 연결되지 않으며 공간 저작 검증용 메타데이터다.
