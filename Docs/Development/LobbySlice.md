...
- 상태: `Accepted`
- 기준일: 2026-08-22
- 관련: [런 결과와 재시작](../Systems/RunCompletion.md), [픽셀 폰트 렌더링](../Systems/PixelFontRendering.md), [런타임 흐름](../Architecture/RuntimeFlow.md), [ADR-0008](../ADR/0008-Dungeon-Scene-Lifetime.md)
...
- 첫 enabled 씬은 `DungeonLobby`이며 제목은 **Bomb Goose**다.
...
- 설정 패널 배경은 `BlackandWhiteUI_117` 직접 참조를 사용한다. 이 87×77 sprite는 중앙 상단 장식과 내부 명암 픽셀까지 하나의 완성 프레임이므로 9-slice 대상으로 보지 않는다. 현재 씬은 디자이너가 조정한 정수 6배 522×462 `Simple` Image를 사용한다. 원본 texture는 Point filter, mipmap off, compression none, Clamp를 유지하며 `Bomb Swap/UI/Configure Lobby Settings Panel Sprite` 메뉴는 importer와 Image 계약만 복구하고 디자이너의 RectTransform 크기·위치를 덮어쓰지 않는다. Unity Game View를 `0.8x`처럼 비정수 배율로 축소한 미리보기는 픽셀 샘플을 다시 보간하므로 선명도 판정은 `1x` 또는 실제 960×600 WebGL canvas에서 한다.
...
- 로비와 pause는 [사용자 설정 계약](../Systems/UserSettingsAndAudio.md)의 같은 factory/presenter를 사용한다. 로비 패널은 씬에 배치하고 pause 패널은 기존 overlay 수명에 맞춰 런타임 생성한다.
...
- 제목은 하나의 TMP 또는 같은 제목 container 아래의 여러 TMP 조각으로 저작할 수 있다. 여러 조각이면 hierarchy 순서와 공백으로 조합한 결과가 **Bomb Goose**여야 한다. main menu `StatusLabel`은 선택 사항이며, 디자이너가 배치한 경우에만 시작·실패 상태 문구를 표시한다. 설정 패널 내부 상태 문구는 별도 필수 참조다.
