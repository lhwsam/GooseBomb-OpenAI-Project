...
- `조작 방법`은 공통 설정 패널을 열어 키보드 조작과 키 변경, 키 설정 초기화, 음량 Slider·화면 흔들림 ON/OFF Button·전체 화면 설정을 제공하며 `돌아가기`로 닫는다. 화면 흔들림 기본값은 켜짐이다. 게임패드 지원은 유지하지만 이 패널에는 표기하지 않는다. 조작 목록은 ScrollRect로 탐색한다.
