...
- 제목은 하나의 TMP 또는 같은 제목 container 아래의 여러 TMP 조각으로 저작할 수 있다. 여러 조각이면 hierarchy 순서와 공백으로 조합한 결과가 **폭탄을 낳는 거위**여야 한다. main menu `StatusLabel`은 선택 사항이며, 디자이너가 배치한 경우에만 시작·실패 상태 문구를 표시한다. 설정 패널 내부 상태 문구는 별도 필수 참조다.
