...
- 중앙 cache는 접근을 막는 논리 `Interactable`이며 cardinal 인접 셀에서 `F`/게임패드 North로 상호작용하면 한 run에서 한 번만 `ROOM TOKENS +3`을 지급한다. 상자 모델은 논리 셀 중심의 바닥 위에 정렬하고, 미수집 상태에는 상자 footprint보다 넓은 바닥 halo와 상승 입자를 표시한다. 수집 뒤에도 상자 모델과 blocker는 유지하고 availability effect와 `F` KeyIcon만 끈다. 일반 전투 `+1`보다 높은 발견 보상이지만 최종 재화 가치·사용처를 확정하지 않는 `Proposed` 임시 점수다.
...
- PlayMode: 실제 전투방의 문 앞 출구 셀이 `Floor`이고 접근·폭탄 설치가 가능하지만 미공개 경계 이동은 차단된다. 그 셀이 실제 폭발 영향에 포함되면 `DestroyedWalls` 없이 해당 문·미니맵만 갱신한다. `DungeonSecret`은 적 없이 입장·왕복 가능하고 중앙 cache 셀은 이동·폭탄을 막는다. 인접 `F` 상호작용 뒤 HUD를 `+3` 갱신하고 상자·blocker를 유지하되 effect·프롬프트를 끄며 재입장에서 재지급하지 않는다.
