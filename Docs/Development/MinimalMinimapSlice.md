...
- 현재 방은 항상 방문 상태이며 `C`와 강조색으로 표시한다.
- 방문한 방은 `V`와 방문색으로 표시한다.
- 방문한 방에 직접 연결됐지만 아직 들어가지 않은 방은 `?`로 표시한다.
...
- browser Console/page error 0과 실제 캡처로 글자·연결·현재 방 강조를 확인한다.
...
- 증거: `Artifacts/Verification/ConnectedTests/20260815-180916-875.json`, `Artifacts/Verification/ConnectedTests/20260815-180936-194.json`, `Artifacts/Verification/20260816-031032-static/`, `Artifacts/Verification/20260816-031100-minimap-connected-web/`.

## 비목표와 롤백

- 미탐색 전체 그래프 공개, 방 종류 아이콘, 보상 상태, 적 위치, 비밀방, fog animation, fast travel, 저장/불러오기.
