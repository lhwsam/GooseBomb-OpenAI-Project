...
- 표준 가상 Gamepad를 브라우저 `navigator.getGamepads()`와 연결 사건에 주입해 스틱·D-pad·South/West/Start/Select의 WebGL→Input System→의미 명령·Core 설치·실패·재시작 경로를 검증하는 전용 smoke를 표준 Web tier에 연결.
...
1. [직선·광역 폭탄 선택 플레이테스트](../Playtesting/DirectionalBombChoiceProtocol.md)에 따라 seed-0 전체 경로를 반복해 추격자 동률 순환, 되돌아가기 피로, `prototype-area`와 `prototype-line`의 선택 차이, 직선 폭탄 방향 표시, 보스 예고·반격·완료·실패·재시작 인지를 함께 관찰한다.
2. [갑옷 적 2회 피격 플레이테스트](../Playtesting/ArmoredEnemyProtocol.md)를 전체 경로 관찰에 포함해 첫 피격의 외형·속도 변화와 두 번째 폭탄 재계획 여부를 기록한다.
3. 원인 요약이 실제 플레이에서 사망 설명력을 높이는지 확인하고, 부족할 때만 공격 이름·위치·시간 같은 상세 정보 중 최소 항목을 추가한다. 여러 층·저장·부활은 계속 범위 밖에 둔다.
...
- 게임패드 binding 구조와 합성 Input System 장치의 왼쪽 스틱·D-pad 네 방향, South/West/Start/Select 의미 명령 변환을 자동 검증했다. WebGL에서는 표준 가상 장치 연결부터 스틱·D-pad 해제, South의 Core 폭탄 설치·자기폭발 실패, West 교체 명령, Start 확정 pause/resume, Select의 실패 런 재시작까지 자동 검증했다. 실제 목표 물리 컨트롤러 연결·장치별 버튼 표기·deadzone·대각선 값·브라우저/OS 차이와 새 직교 축 우선 조작감은 수동 플레이가 남아 있다.
...
- 같은 post-commit WebGL 빌드에 표준 가상 Gamepad를 연결한 최종 Edge 151 smoke 10/10이 통과했다. 왼쪽 스틱 `East → None`, D-pad `North → None`, West 교체 명령, Start 확정 pause/resume, South의 실제 `prototype-cross` 5회 설치·자기폭발 실패, Select의 페이지 reload 없는 새 start-safe 런 재시작과 browser Console/page error 0을 확인했고 `PAUSED` 화면을 캡처했다. 첫 시도는 엔진 준비 뒤 연결 사건이 없어 입력이 전달되지 않은 실패로 보존했고, 실제 `gamepadconnected` 발생 뒤 8/8 기초 경로와 10/10 Select 확장 경로가 차례로 통과했다. 증거 `Artifacts/Verification/20260815-074000-gamepad-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-074200-gamepad-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-075000-gamepad-select-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-075000-gamepad-select-browser/gamepad-paused.png`.
