...
- 첫 방 `prototype-combat-loop` ScriptableObject를 TestSandbox 격자·고정 벽·spawn의 단일 저작 원본으로 연결하고 씬 중복 수치 제거.
- 방 연결성, 서로 다른 첫 이동의 퇴로 2개, 출구 경계, 닫힌 cardinal 유도 경로와 씬 장애물 표현 일치를 builder·validator·테스트로 검증.
- 첫 내부 관찰 세션의 고정 build, 비유도 시작 안내, 시도별 관찰표, 직후 인터뷰, 유지·변경·제거 후보 기준과 익명 결과 템플릿 정의.
...
- `PrototypeCombatLoop.asset`은 11×9 방의 플레이어 `(0, 0)`, 추격자 `(1, -1)`, 중앙 십자 고정 벽 4개, 남·북 출구, 좌·우 퇴로 anchor, 중앙 8셀 유도 순환 경로를 소유한다.
- `TestSandboxContext`는 격자 크기·셀 크기·blocked cell을 방 자산에서 읽는다. spawn과 내부 장애물 Transform은 표현이며 validator가 저작 셀과 일치하는지 확인한다.
...
- PlayMode 전체 47개가 `GridSpace`, room asset→격자·spawn·blocked cell 연결, cardinal 입력 해석, 실제 Input System 키→명령→공유 격자 플레이어·추격자 이동, 접촉 피해·공유 무적·같은 프레임 폭발 사망 우선순위, 폭탄 설치·한 번 탈출·재진입 차단·fuse 폭발·적 사망·방 클리어, Transform 보간, pooled 표현과 property block 생명주기, 저작 장애물 차단, probe 초기화 순서, focus reset, 재구독 계약과 하네스 발견성을 검증한다.
- TestSandbox의 네 내부 장애물은 Transform/Collider가 아니라 방 ScriptableObject의 명시적 논리 blocked cell로 저작되어 있다.
...
1. `Docs/Playtesting/FirstCombatProtocol.md`에 따라 첫 수제 전투방에서 사람의 관찰 플레이테스트를 실행하고 익명 증거를 기록한다.
...
- `Tools/Verify.ps1 -StaticOnly`: 통과. Markdown 링크, 스킬 4종, asmdef 5종, Core 금지 API 검사. 최신 기록 산출물 `Artifacts/Verification/20260814-103449-static/`.
...
- PlayMode 전체 회귀: `BombSwap.Unity.Tests` 47개 통과, 실패/건너뜀/불확정 0. room asset의 격자·blocked cell·spawn 연결과 기존 입력 이동·추격·접촉 피해·공유 무적·폭탄·방 클리어·표현 생명주기 포함.
- `PrototypeContentValidator`: 전투방 Core 변환, `prototype-combat-loop` 참조, TestSandbox spawn/placeholder·내부 장애물 표현 셀 일치와 기존 Input Actions·폭탄·vitals·추격자·session·카메라·조명·Build Settings 검증 오류 0.
- TestSandbox 실제 재생과 Scene View 시각 확인: `Z` 설치 성공 뒤 플레이어 셀의 검은 구형 폭탄 placeholder, 11×9 격자·경계 벽·네 장애물·플레이어를 탑다운 구도에서 식별.
- Development WebGL 방 저작 전체 빌드: TestSandbox 단일 씬으로 성공. 140,521,752 bytes, 291.473초, 오류 0. 전체 셰이더 재컴파일로 설치된 Sentis·vendor·TextMeshPro 관련 경고 359개가 보고됐으며 방 코드 오류는 없었다.
...
- 최신 WebGL 검증 증거: `Artifacts/Verification/20260814-102330-web-connected/` (Git 제외). 빌드 후 자동 생성된 URP/ProjectSettings/Burst 부산물은 작업 diff에서 제거했다.
