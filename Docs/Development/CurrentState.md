...
- 같은 post-commit WebGL 빌드에 표준 가상 Gamepad를 연결한 최종 Edge 151 smoke 11/11이 통과했다. 왼쪽 스틱 `East → None`, D-pad `North → None`, West 교체 명령, Start pause 중 400ms 논리 셀·동쪽 step·South 설치 불변과 재개 뒤 유지 스틱 한 셀 적용·중립 정지, South의 실제 `prototype-cross` 5회 설치·자기폭발 실패, Select의 페이지 reload 없는 새 start-safe 런 재시작과 browser Console/page error 0을 확인했고 `PAUSED` 화면을 캡처했다. 첫 시도는 엔진 준비 뒤 연결 사건이 없어 입력이 전달되지 않은 실패로 보존했고, 실제 `gamepadconnected` 발생 뒤 8/8 기초, 10/10 Select, 11/11 pause 확장 경로가 차례로 통과했다. 증거 `Artifacts/Verification/20260815-074000-gamepad-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-074200-gamepad-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-075000-gamepad-select-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-075300-gamepad-pause-browser/gamepad-smoke.json`, `Artifacts/Verification/20260815-075300-gamepad-pause-browser/gamepad-paused.png`.
