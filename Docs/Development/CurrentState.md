...
- 현재 고정 WebGL 첫 시도 시작방에서 키 해제 정지, 빠른 `상→우` 반복과 벽 모서리 전환을 재확인해 PT-20260814-01의 방향 응답성 판정을 갱신한다.
...
1. [직선·광역 폭탄 선택 플레이테스트](../Playtesting/DirectionalBombChoiceProtocol.md)에 따라 첫 시도 시작방에서 키 해제·빠른 `상→우` 반복·벽 모서리 전환을 먼저 재확인한 뒤, seed-0 전체 경로를 반복해 추격자 동률 순환, 되돌아가기 피로, `prototype-area`와 `prototype-line`의 선택 차이, 직선 폭탄 방향 표시, 보스 예고·반격·완료·실패·재시작 인지를 함께 관찰한다.
