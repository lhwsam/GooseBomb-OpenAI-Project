...
1. [seed-0 전체 경로 계약](DungeonFullPathSmokeSlice.md)을 사람 플레이로 반복해 추격자 동률 순환, 되돌아가기 피로, `prototype-area`와 `prototype-line`의 선택 차이, 직선 폭탄 방향 표시, 보스 예고·반격·완료·실패·재시작 인지를 함께 관찰한다.
...
- 던전 9개 씬 최신 Development WebGL 빌드는 137,754,003 bytes, 122.300초이며 BuildReport 오류 0, 전체 shader 재컴파일에 따른 기존 Sentis·vendor·TextMeshPro 범주의 경고 351건이다. 이 development 수치와 빌드 시간을 release 성능이나 cold build 예산으로 해석하지 않는다. 실제 배포 예산과 미사용 AI Inference·vendor 패키지 정리는 사람 수직 슬라이스 검증 이후 별도 결정이 필요하다.
