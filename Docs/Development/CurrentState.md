...
- 기준일: 2026-08-14
- 단계: 기본 십자·3×3 광역 두 슬롯 WebGL 자동 검증 완료, 비교 플레이테스트 준비
...
- EditMode 테스트 170개가 하네스 발견성, 좌표·격자·시계와 cardinal 인접, actor 식별, 십자·광역 폭탄 설치·폭발·벽·연쇄, 두 슬롯 독립 쿨타임·실패 미소비·교체 경계·주입 시계 정지, 플레이어 명령과 frame 연속 진행·해제 즉시 정지·빠른 방향 반복·다중 셀 경계·점유 전이·설치자 한정 통과, 폭발/접촉 피해 원인·공유 무적 경계, 추격자 결정론·cadence·방향 유지·폭탄 차단·단일 피격, 방 경계·연결성·퇴로·유도 경로 계약을 검증한다.
- `GridSpace`는 임의 원점·양수 셀 크기의 격자↔3D XZ 변환을 제공하고 Y를 표현 높이로 분리한다.
- PlayMode 전체 67개가 `GridSpace`의 정수·연속 좌표 변환, room asset→격자·spawn·blocked cell 연결, cardinal 입력과 새 직교 방향 우선의 키 겹침, 실제 Input System 유지·해제·6회 `North/East` 단타의 같은 frame Core 위치·Transform 반영, 실제 `X` 교체와 광역 `Z` 설치·3×3 결과·HUD snapshot 표시, 공유 격자 플레이어·추격자 이동, 접촉 피해·공유 무적·같은 프레임 폭발 사망 우선순위, 폭탄 설치·한 번 탈출·재진입 차단·fuse 폭발·적 사망·방 클리어, 방 전환 pending·마지막 방 무전환, pooled 표현과 property block 생명주기, 저작 장애물 차단, probe 초기화 순서, focus reset, 재구독 계약과 하네스 발견성을 검증한다.
...
- 최신 WebGL에서 기본 십자와 3×3 광역의 위치 선택·자기 위험·방별 유용성 차이를 사람이 비교한다.

## 바로 다음 권장 작업

1. 최신 WebGL에서 십자는 통로·cardinal 정렬, 광역은 열린 공간·주변 군집에 실제로 다른 설치 위치를 만드는지 비교한다.
2. 파괴 가능 벽이나 폭탄 상호작용 블록을 추가해 두 폭탄의 전파·차단 차이를 읽을 수 있는 다음 플레이테스트 조건을 만든다.
3. 세 방에서 슬롯별 설치 위치·교체 직후 설치·자기 피격을 관찰해 광역 fuse 1.75초와 설치 쿨타임 2.5초를 유지·변경·제거로 판정한다.
...
- 광역 폭탄 연결 후 EditMode 170개와 PlayMode 67개 전체 통과, 실패/건너뜀/불확정 0. 대상 Core 29개와 `PrototypePlayerControllerTests` 22개도 별도로 통과했으며 `PrototypeContentValidator`는 두 모양·로드아웃·세 씬·레거시 제거를 오류 0으로 검증.
- 광역 폭탄 Development WebGL 빌드 성공: 140,817,654 bytes, 78.902초, 오류 0, 설치된 패키지·셰이더 기존 범주의 경고 359개. 연결 실행 요약과 빌드는 `Artifacts/Verification/20260814-165049-area-bomb-web-connected/`에 기록.
- 최신 Edge headless smoke: load, canvas focus, 기본 정의 설치, `X`로 두 번째 슬롯 선택, `prototype-area` 성공 설치, 3방 시퀀스, 6회 `North/East` 단타, 마지막 방 자기 폭발, resize, gameplay probe와 browser Console/page error 0을 모두 통과. 같은 산출물의 `browser-smoke.json`에 기록.
- 실제 WebGL 캡처에서 하단 좌측 2번 `prototype-area` 활성 슬롯·쿨타임, 보라색 광역 폭탄과 기존 십자 폭발의 표현 구분을 확인. 증거는 같은 산출물의 `webgl-gameplay.png`다.
- 최신 정적 검증 `Artifacts/Verification/20260814-165637-static/`, `node --check Tools/WebGLSmoke.mjs`, `WebGLStaticServerTests.mjs` 통과.
