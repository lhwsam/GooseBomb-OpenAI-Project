...
- 보스는 체력 10이며 과열 한 번에 서로 다른 플레이어 폭탄으로 최대 2피해를 받는다.
...
- Core: 3 phase 정의, typed pattern tuning, 원자 시퀀스, 다중 이동 결과, 순차 투척 계획·flight 데이터, 과열 상한, 자폭병 직접 피해·해결 gate.
...
- `BossBattleDefinition`이 최대 체력 10, Two/LastStand 임계 7/2, 과열 피해 상한 2와 보스 폭탄 두 정의를 검증한다.
...
- 플레이어 폭탄은 Overheat Recovery에서만 BombId당 1·회차당 2, 자폭병은 상태 무관 BombId당 1이다.
...
- 최종 EditMode 전체 `328/328`, 실패·건너뜀 0: `Artifacts/Verification/ConnectedTests/20260819-105908-860.json`.
- 최종 PlayMode 전체 `132/132`, 실패·건너뜀 0: `Artifacts/Verification/ConnectedTests/20260819-105933-548.json`. 보스 돌진·parity 피해가 `BossPattern` source로 발행되는 회귀를 포함한다.
- StaticOnly 통과: `Artifacts/Verification/20260819-200049-static/summary.json`.
- 최종 11씬 Development WebGL 빌드 성공: 138,799,553 bytes, 72.131초, 오류 0·안내 경고 3. 증거는 `Artifacts/Verification/20260819-2001-connected-web-final/`이다.
- Edge keyboard smoke `46/46`이 1페이즈 과열 2회, 일회성 자폭병, 2페이즈 과열 2회, LastStand와 처치를 포함한 전체 seed-0 경로를 통과했다. 과열마다 서로 다른 플레이어 폭탄 2개가 피해를 주고 정확한 이동 목적지 ghost는 나타나지 않았다.
- 플레이테스트 사건 2,485개 분석과 가상 표준 Gamepad smoke `14/14`, 두 브라우저 실행의 Console/page error 0을 확인했다.
...
2. 과열당 2피해 시도율, 자폭병 유도율, phase별 피격, 전투 시간, parity 안전 칸 재사용을 기록한다.
