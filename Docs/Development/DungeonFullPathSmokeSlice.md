...
4번 전투방에서 플레이어가 `(-3,-4)`에 있고 추격자가 x=1 열에 있을 때, 과거 국소 Manhattan 거리와 기존 방향/`North → East → South → West` 동률 규칙 때문에 추격자가 `(1,-3)`, `(1,-4)`, `(1,-5)` 사이를 반복했다. 당시 자동 smoke는 플레이어를 `(-3,-5)`로 한 칸 이동해 동률을 깨고 계속 진행했다.

후속 gameplay change는 재계획 시 BFS 거리장과 committed step의 거리 감소 조건을 채택했다. 막힌 포켓보다 실제 최단 가지를 선택하고 계획 거리를 늘리는 두 번째 step 전에 재계획하는 Core 회귀 테스트로 같은 원인의 반복을 차단한다. smoke도 기존 `(-3,-5)` 위치 이동과 두 번째 폭탄 대기를 제거했다. 현재는 `(0,-5)`에 첫 광역 폭탄을 설치하고 `(0,-4)`에서 추격자를 범위 안으로 유도한 뒤 `(3,-4)`로 이탈해 첫 폭발로 클리어한다.
