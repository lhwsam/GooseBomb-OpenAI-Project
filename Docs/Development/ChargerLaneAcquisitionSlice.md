...
- 플레이어 `(-3,-2)`, 추격자 `(3,2)`, 돌진형 `(-1,1)`에서 시작한다. 돌진형은 기본 배치와 seed-0 회전 뒤 동쪽 입장 모두에서 즉시 정렬되지 않고 최소 한 번의 Track 이동으로 정렬 셀을 획득해야 한다.
...
- 2026-08-20 사람 플레이에서 seed-0 `Pillars` 입장 직후 돌진 공격이 시작돼 당황스럽다는 관찰이 나왔다. 같은 빌드의 사건 로그는 방 준비 19ms 뒤 Telegraph, 약 1.01초 뒤 충돌 피해를 보여 관찰과 일치했다. 전역 예고·돌진 수치는 유지하고 저작 spawn만 `(0,1) → (-1,1)`로 옮겨 회전 입장 축을 벗어나게 했으며, 새 체감은 사람 재검증 대상으로 남긴다.
- 변경 뒤 Chromium smoke는 seed-0 회전 `Pillars` 준비 다음 첫 돌진형 관련 행동이 `charger-track-moved`임과 방 클리어를 확인했다. 다른 적 점유나 폭탄 선행 처치에 따라 한 browser run에서 모든 돌진 상태가 나타나지 않을 수 있으므로 Telegraph·Charge 이동/목표 충돌·Recover의 모든 분기는 전체 EditMode `330/330`·PlayMode `133/133`이 소유한다.
