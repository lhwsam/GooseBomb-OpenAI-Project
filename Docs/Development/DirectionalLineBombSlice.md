...
첫 폭탄 보상의 오른쪽 후보를 긴 십자가 아니라 플레이어가 바라보는 앞쪽으로만 길게 폭발하는 `prototype-line`으로 교체한다. 광역 폭탄과 직선 폭탄이 서로 다른 방 구조와 적 정렬에서 다른 설치 위치를 만들 수 있는 상태까지 구현한다.
...
보상 catalog의 두 후보 순서는 `prototype-area`, `prototype-line`이다. 현재 수치는 fuse 2.25초, 앞쪽 범위 5(원점 포함 총 6셀), 설치 쿨타임 2.25초와 표시 시간 0.25초다. 범위는 기존 3에서 5로 상향했으며, 이 수치와 직선 폭탄을 더 빠르게 만들지는 사람 플레이테스트로 비교할 `Proposed` 값이다.
