...
- 플레이어가 투척병의 Throw 동작을 보고 이동을 준비하고, 착탄 뒤 공개되는 범위를 피해 퇴로를 바꾸거나 적 폭탄을 자신의 폭발로 연쇄시킬 수 있다.
...
- Telegraph 진입 시 세 목표를 함께 고정하지만 착지 셀은 표시하지 않으며 경고 중에는 재조준하지 않는다. Throw 애니메이션과 본체 pulse는 유지한다.
- 세 폭탄은 같은 시각에 서로 다른 포물선으로 비행하며 이 구간에는 목표 셀과 폭발 범위를 표시하지 않는다. 공용 `BombSimulation`에 각각 성공 착탄한 뒤에만 현재 Core 폭발 미리보기 범위를 fuse 동안 표시하고, 실제 폭발한 폭탄의 범위만 제거한다. 따라서 플레이어 폭발과 같은 벽 판정과 고정 연쇄 지연을 사용한다.
...
- Unity: 전용 정의 에셋, 적/폭탄 프리팹, 룸 정의, `ThrowerLanesPlaytest` 씬, 이동·Throw·비행·착탄 표현과 착탄 전 목표 셀 비표시
...
- `Artifacts/Verification/20260820-063300-thrower-rotation-final-web/`의 전용 Development WebGL은 138,713,132 bytes·63.378초·오류 0·안내 경고 3건으로 성공했다. 당시 `Tools/ThrowerWebGLSmoke.mjs`의 `8/8` 검사는 화면의 착지 셀 표시가 아니라 텔레메트리로 첫 중앙·하단 좌우 목표 잠금 뒤 두 번째 중앙·상단·좌측 잠금으로의 측면 교체를 확인했고, 세 비행·공용 폭탄 착탄, 플레이어 폭탄의 `Chain` 기폭과 Console/page 오류 0도 확인했다.
...
- 착지 셀을 미리 보여주지 않아도 0.3초 Throw 동작과 본체 pulse, 포물선 비행이 공격 시작을 충분히 알리고 착탄 뒤 범위가 회피 가능한 시점에 읽히는가.
