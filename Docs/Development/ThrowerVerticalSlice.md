...
- 세 폭탄은 같은 시각에 서로 다른 포물선으로 비행하며 이 구간에는 폭발 범위를 표시하지 않는다. 공용 `BombSimulation`에 각각 성공 착탄한 뒤에만 현재 Core 폭발 미리보기 범위를 fuse 동안 표시하고, 실제 폭발한 폭탄의 범위만 제거한다. 따라서 플레이어 폭발과 같은 벽 판정과 고정 연쇄 지연을 사용한다.
