...
`DungeonGraph`의 모든 전투 노드에 기존 네 수제 전투방 중 하나와 회전을 결정적으로 배정한다. 그래프 연결 방향과 실제 잠재 출구 방향은 일치해야 하고, 같은 seed·그래프·카탈로그에서 재현 가능해야 한다.
...
- `prototype-combat-loop`, `lanes`, `pillars`, `armor`는 중앙 북 `(0,4)`, 동 `(5,0)`, 남 `(0,-4)`, 서 `(-5,0)` 잠재 출구를 가진다.
- 같은 방향의 출구 중복은 `CombatRoomDefinition` 생성 시 거부한다.
- Editor builder가 네 자산을 Unity 직렬화로 동기화하고 validator가 cardinal 네 방향을 재확인한다.
...
- 128개 seed 배정 다양성, 카탈로그 순서 무관 재현, 정의별 사용 균형과 실패 경계.
- 모든 배정의 활성 출구가 그래프 연결과 회전된 저작 출구에 일치.
- 실제 네 room asset의 Editor content validator 오류 0.
- 전체 EditMode·PlayMode 회귀와 Console Error 0.
- `Tools/Verify.ps1 -StaticOnly` 통과. 실행 중 Editor 잠금 시 Full 명령 대신 연결된 동일 구성요소 증거를 남긴다.

## 롤백

배정 Core 파일, 회전 값, 방 정의의 방향 중복 불변식, 네 room asset의 cardinal 출구, builder·validator·테스트와 이 문서를 함께 되돌린다. `DungeonGenerator`의 `prototype-tree-v1` 출력은 변경하지 않는다.
