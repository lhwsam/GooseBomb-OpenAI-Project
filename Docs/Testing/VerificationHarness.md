...
`Tools/WebGLSmoke.mjs`는 `Tools/WebGLStaticServer.mjs`의 정적 서버와 Playwright를 사용한다. 같은 서버 모듈은 수동 관찰용 `Tools/ServeWebGL.mjs`에서도 사용하므로 자동·수동 실행의 MIME·압축·경로 경계가 갈라지지 않는다. 게임플레이 probe는 먼저 `probe-ready`와 현재 방별 `room-ready-<room-id>`를 보내 Unity 런타임, 방 권위와 입력 구독이 준비됐음을 알린다. 이후 `move`, 입력 명령 `move-direction-*`, 실제 frame 위치 변화 `move-motion-direction-*`, 논리 셀 경계 `move-step-direction-*`, `chaser-moved`, `charger-telegraph`, `charger-charge`, `charger-moved`, `armored-moved`, `armored-broken`, `armored-died`, `place-bomb`, 두 `place-bomb-definition-*`, 성공한 `active-bomb-slot-1`, `destructible-wall-destroyed`, `player-contact-damaged`, `contact-escape-moved`, `bomb-exploded`, `player-damaged`, `player-explosion-damaged`, `enemy-died`, `room-cleared`, `room-transition-started`, `swap-bomb`, `pause-resume`, `audio-unlocked` 사건을 문자열 또는 `{ name: string }` 형태로 제공해야 한다.

현재 TestSandbox 개발 빌드는 `PrototypeInputHarnessProbe`와 `BombSwapHarness.jslib`로 이 배열을 제공한다. `probe-ready`와 방 준비 표식은 `PrototypeGameSession`이 InputReader 구독까지 끝낸 뒤에만 기록한다. Playwright는 canvas focus와 첫 방 준비를 확인하고, `move`까지 `W` 유지 → `Z`로 기본 정의 설치 → 접촉 피해까지 대기 → `contact-escape-moved`까지 `A` 유지 → 첫 폭발 확인 → `X` 성공과 2번 슬롯 확인 → `Z`로 `prototype-area` 광역 정의 설치 → `Esc` 두 번 순서로 진행한다. 이 시점의 화면을 기본 `webgl-gameplay.png` 또는 `--screenshotPath` 위치에 저장한다. 첫 `room-transition-started`와 평행 통로 준비 뒤 파괴 가능 블록이 보이는 화면을 `webgl-destructible-walls.png` 또는 `--wallScreenshotPath`에 저장하고 현재 광역 폭탄을 `Z`로 설치한다. 두 대각선 블록의 실제 확정 파괴 사건 `destructible-wall-destroyed`, 두 번째 전환과 엇갈린 기둥 준비까지 기다린다. 세 번째 방에서는 `ArrowUp/ArrowRight`를 여섯 번 교대해 누르고 각 release 전에 대응하는 실제 frame 이동이 같은 순서로 와야 통과한다. 이어 전체 사건 배열에서 `charger-telegraph → charger-charge → charger-moved` 순서를 확인하고, 서쪽 논리 이동 두 칸으로 최초 돌진선에서 벗어난 뒤 폭탄을 설치해 차저 접촉 무적과 겹치지 않은 실제 자기 폭발 피해를 확인한다. 기본 smoke는 네 번째 방을 로드하지 않고 기존 3방 회귀를 유지하며, 네 번째 방은 전용 갑옷 smoke와 Build Settings·scene validator로 분리 검증한다. 고정 지연 대신 Core 사건과 방별 준비 표식으로 동기화해 느린 headless WebGL 프레임과 씬 로드의 입력 오탐을 피한다. `swap-bomb`은 입력 수신만 뜻하고 `active-bomb-slot-1`은 Core 교체 성공을 뜻한다. `player-contact-damaged`와 `player-explosion-damaged`는 적용된 피해 원인이 각각 적 `ActorId`와 `BombId`로 확정됐을 때만 기록된다. 나머지 이동·설치·폭발·벽 파괴·사망·클리어 표식도 실제 Core 상태 전이에서만 발생한다. `audio-unlocked`는 사용자 입력 수신 marker일 뿐 실제 오디오 출력 검증을 대체하지 않는다.
