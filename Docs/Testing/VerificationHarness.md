...
Editor가 열려 있어 batchmode Web tier를 실행할 수 없을 때는 `Assets/Game/Editor/BuildAutomation/ConnectedWebGLBuildHarness.cs`를 사용한다. 일반 연결 작업은 `Bomb Swap/Verification/Build Development WebGL Connected` 메뉴가 시각이 포함된 artifact 경로를 만들고 예약한다. 자동화에서 경로를 직접 제어해야 하면 공식 Unity MCP `Unity_RunCommand`로 짧은 빌드에는 `BuildDevelopment(artifactsDirectory, buildPath)`, MCP 제한 시간을 넘길 수 있는 빌드에는 `ScheduleDevelopment(artifactsDirectory, buildPath)`를 호출한다. 예약 경로는 컴파일된 `EditorApplication.update` callback으로 도구 응답과 분리해 실행을 이어가고 `webgl-build-status.txt`에 `Scheduled → Running → Passed | Failed`를 기록한다. 예약 뒤 스크립트 재컴파일이나 domain reload를 일으키지 말고 status가 종료 상태인지 확인한다. 모든 경로는 content validator를 먼저 실행하고 enabled scene 전체로 Development WebGL을 빌드한 뒤 `webgl-build-report.json`과 `WebGLBuild/`을 남기며 Editor를 종료하지 않는다. 최종 status, 성공 JSON과 `index.html`을 확인한 뒤 같은 `Tools/WebGLSmoke.mjs`를 별도로 실행해야 하며, 연결 빌드만으로 Web tier 전체 통과를 주장하지 않는다.
...
Gates 구간은 graph 연결 방향에 따라 저작 player spawn 대신 경계 입장 셀에서 시작할 수 있다. seed-0 smoke는 `room-ready-prototype-combat-gates` 직후 실제 현재 셀에서 오른쪽 아래 `(3,-3)`으로 Z축 우선 이동해 자폭병을 오른쪽 우회로 `(3,-1)`까지 끌어낸다. 그 셀 사건 뒤 플레이어가 왼쪽 아래 `(-1,-2)`로 X축 우선 이동하면 자폭병의 최단 추적이 아래 유도 anchor `(0,-2)`를 통과하며, 여기서 `self-destruct-warning-chase`·`self-destruct-armed`·`self-destruct-telegraph`를 확인한다. 플레이어는 한 칸 떨어진 안전 셀 `(-1,-3)`으로 이탈해 0.75초 fuse를 실제로 기다리고, `self-destruct-detonated → bomb-exploded-definition-prototype-self-destruct-blast → self-destruct-died`와 진입 시점보다 정확히 하나 이상 증가한 `destructible-wall-destroyed`를 확인한다. 그 뒤 서쪽에서 접근 중인 추격자와 교차하지 않도록 동쪽 우회로에서 추격자를 처치하고 `room-cleared`를 확인한다. 이 순서는 자폭병이 스스로 저작 anchor를 찾아가는 이전 동작에 의존하지 않고 플레이어의 2단계 유인·경고·정지·환경 파괴를 각각 관찰한다.
