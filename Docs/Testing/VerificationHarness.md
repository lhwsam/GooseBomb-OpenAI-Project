...
- browser 실행 시 `browser-smoke.json`, `gamepad-smoke.json`, `gamepad-paused.png`, `browser-smoke.log`.
...
현재 던전 개발 빌드는 `PrototypeInputHarnessProbe`, `PrototypeDungeonRunHost`, `BombSwapHarness.jslib`로 이 배열을 제공한다. Playwright는 `DungeonStart` canvas focus와 `dungeon-room-ready-1-start-safe`를 확인한 뒤 오른쪽 키 유지 중 브라우저 `blur`를 발생시켜 `move-direction-none`과 300ms 셀·motion 정지를 확인한다. 누락 key-up 상태로 `focus`를 복구해도 이동이 되살아나지 않아야 하며, 이어지는 `Esc` pause 성공이 입력 map 복구까지 증명한다. 그 뒤 `PAUSED` 화면을 캡처하고 방향키와 `Z`가 현재 셀·frame motion·폭탄 설치 수를 바꾸지 않는지 400ms 동안 확인한 뒤 재개한다. 첫 체력 probe run에서는 시작방 자기 폭발로 `5→4`, 첫 전투방 준비도 `4`임을 확인한 뒤 추격자 접촉 실패와 `R` 재시작으로 새 run `5`를 증명한다. 그 새 run에서 seed-0 서쪽 출구까지 저작 장애물을 다시 우회한다. 첫 전투방의 `dungeon-room-ready-2-combat-active` 뒤 겹친 두 키의 최신 직교축 우선과 빠른 방향 교대를 frame motion으로 검증하고, 시작 슬롯의 십자 폭탄을 두 번 설치해 두 적을 처치하고 `room-cleared`를 만든다. 이어 북쪽 `BombReward`의 `dungeon-room-ready-3-bomb-reward-safe`에서 왼쪽 후보를 수집해 `bomb-reward-selected-prototype-area`를 확인하고 그 방에서 슬롯 2를 활성화한다. 남쪽으로 돌아온 첫 전투방의 `dungeon-room-ready-2-combat-cleared`와 적 사건 미발생, 보상방 재진입을 확인한 뒤 주 경로 4번 방에서 추가 교체 입력 없이 광역 폭탄을 설치해 활성 슬롯의 run persistence를 검증한다. 이후 4번 회전 루프와 5번 중앙 게이트 전투방을 유지된 광역 폭탄과 추격자 셀 marker로 클리어한다. 체력이 방마다 회복되지 않으므로 추격자 거리·Area 2.5초 쿨타임·폭탄 점유를 기준으로 선제 설치하고 폭발 반경 밖 셀로 이탈한다. 게이트 방을 클리어하면 8번 Recovery leaf를 우회한다. 입장만으로 체력이 바뀌지 않는지 확인하고 중앙 pickup까지 안전 경로로 이동해 손상된 현재 체력을 최대 체력 이하에서 `+2` 회복한 뒤 `player-health-recovered-2`와 `recovery-consumed-room-8`을 기다린다. 같은 방에서 다시 회복되지 않는지 확인하고 5번 클리어 방으로 돌아와 6번 보스 전실과 7번 보스방에 진입한다. 첫 보스 Telegraph에서 `boss-cell-x-0-z-1`과 `boss-move-target-x-1-z-1`을 확인하고 목적지 영향 범위에 광역 폭탄을 먼저 설치한다. 이후 네 `boss-moved` 셀이 `(1,1) → (1,0) → (1,-1) → (0,-1)`과 일치하고 `boss-move-blocked`가 없으며, 첫 선행 설치와 세 Recovery 설치가 각각 `boss-damaged`를 만든 뒤 격파와 `run-completed`로 이어져야 한다. 완료 뒤 `R` 재시작을 확인하고, 새 안전방에서는 0번 십자 폭탄 자기 폭발 5회로 새 run 활성 슬롯 초기화, `player-died`, `run-failed`, `run-failed-cause-bomb-explosion`과 실패 화면의 `CAUSE: BOMB EXPLOSION`을 검증한 뒤 다시 재시작한다. 한 세션의 graph 전환과 commit은 체력 probe의 첫 전투방 진입과 Recovery 왕복을 포함해 각각 11회, 주 경로 일반 전투 클리어는 3회다. 마지막으로 viewport resize와 Console/page error 0을 확인하고 pause·게이트 방·회복방·완료·보스 예고·실패 화면을 각각 캡처한다. 방 준비·이동·전환은 고정 지연이 아니라 가장 최근 전투방 이후의 현재 논리 셀, 적 셀과 scene commit 사건으로 동기화하며 fuse와 쿨타임 경계에만 명시적 시간을 사용한다. `swap-bomb`은 입력 수신만 뜻하고 `active-bomb-slot-1`은 Core 교체 성공을 뜻한다. `audio-unlocked`는 사용자 입력 수신 marker일 뿐 실제 오디오 출력 검증을 대체하지 않는다.
