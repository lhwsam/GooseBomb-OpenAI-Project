...
`Tools/ThrowerWebGLSmoke.mjs`는 `ThrowerLanesPlaytest` 한 씬만 사용하는 별도 development 빌드에서 투척병 연결을 검증한다. 시작 플레이어 `(0,-2)`와 다른 중앙 `(0,0)`·좌우 `(-3,-2)/(3,-2)` 첫 목표 3개의 동시 예고를 확인하고 시작 셀에 즉시 범위 2 십자 폭탄을 설치해 중앙 착탄보다 먼저 fuse를 예약한다. 세 `thrower-telegraph-x-*` 뒤 `thrower-bomb-launched`와 `thrower-bomb-armed-definition-prototype-thrower-blocker`가 각각 3번 발생하고 `thrower-bomb-detonated-by-chain`으로 이어져야 한다. 첫 폭탄들이 해결되고 다음 사격 anchor로 이동한 뒤에는 중앙 `(0,0)`을 유지하면서 측면이 `(0,4)/(-4,1)`로 교체된 두 번째 예고를 추가로 기다린다. 전체 순서와 browser Console/page error 0을 구조화된 JSON으로 남긴다. 이 검사는 세 저작 목표 잠금·세 비행→논리 착탄·공용 연쇄·연속 volley 비반복을 증명하지만 사람의 예고 인지, 퇴로 선택과 위협도는 판정하지 않는다.
