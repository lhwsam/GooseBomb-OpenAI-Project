...
현재 던전 개발 빌드는 `PrototypeInputHarnessProbe`, `PrototypeDungeonRunHost`, `BombSwapHarness.jslib`로 이 배열을 제공한다. Playwright는 `DungeonStart` canvas focus와 `dungeon-room-ready-1-start-safe`를 확인한 뒤 오른쪽 키 유지 중 브라우저 `blur`를 발생시켜 `move-direction-none`과 300ms 셀·motion 정지를 확인한다. 누락 key-up 상태로 `focus`를 복구해도 이동이 되살아나지 않아야 하며, 이어지는 `Esc` pause 성공이 입력 map 복구까지 증명한다. 그 뒤 `PAUSED` 화면을 캡처하고 방향키와 `Z`가 현재 셀·frame motion·폭탄 설치 수를 바꾸지 않는지 400ms 동안 확인한 뒤 재개한다. 이어 seed-0 서쪽 출구까지 저작 장애물을 우회한다. 첫 전투방의 `dungeon-room-ready-2-combat-active` 뒤 겹친 두 키의 최신 직교축 우선과 빠른 방향 교대를 frame motion으로 검증하고, 시작 슬롯의 십자 폭탄을 두 번 설치해 두 적을 처치하고 `room-cleared`를 만든다. 이어 북쪽 `BombReward`의 `dungeon-room-ready-3-bomb-reward-safe`에서 왼쪽 후보를 수집해 `bomb-reward-selected-prototype-area`를 확인하고, 남쪽으로 돌아온 첫 전투방의 `dungeon-room-ready-2-combat-cleared`와 적 사건 미발생, 보상방 재진입을 확인한다. 이후 주 경로의 4번 회전 루프와 5번 중앙 게이트 전투방을 선택한 광역 폭탄과 추격자 셀 marker로 클리어한다. 게이트 방은 진입 직후 전용 screenshot을 남기고 서쪽 우회로에서 적을 유인한 뒤 6번 보스 전실과 7번 보스방에 진입한다. 보스 격파와 `run-completed` 뒤 `R` 재시작을 확인하고, 새 안전방에서 자기 폭발 5회로 `player-died`, `run-failed`, `run-failed-cause-bomb-explosion`과 실패 화면의 `CAUSE: BOMB EXPLOSION`을 검증한 뒤 다시 재시작한다. 한 세션의 graph 전환과 commit은 각각 8회, 전투 클리어는 3회다. 마지막으로 viewport resize와 Console/page error 0을 확인하고 pause·게이트 방·완료·보스 예고·실패 화면을 각각 캡처한다. 방 준비·이동·전환은 고정 지연이 아니라 가장 최근 전투방 이후의 현재 논리 셀, 적 셀과 scene commit 사건으로 동기화하며 fuse와 쿨타임 경계에만 명시적 시간을 사용한다. `swap-bomb`은 입력 수신만 뜻하고 `active-bomb-slot-1`은 Core 교체 성공을 뜻한다. `audio-unlocked`는 사용자 입력 수신 marker일 뿐 실제 오디오 출력 검증을 대체하지 않는다.
...
`Tools/GamepadWebGLSmoke.mjs`는 페이지 script보다 먼저 표준 매핑 가상 Gamepad를 `navigator.getGamepads()`에 주입하고 Unity 입력 초기화 뒤 실제 `gamepadconnected` 사건을 발생시킨다. 왼쪽 스틱 East와 중립, D-pad North와 해제가 각각 `Move`와 `Move(None)`을 만들고, South가 실제 시작 폭탄을 설치하며 West가 교체 명령, Start가 확정 pause 진입·재개를 만드는지 확인한다. 결과는 `gamepad-smoke.json`과 `gamepad-paused.png`에 browser Console/page error와 함께 기록한다. 이 검사는 브라우저 Gamepad API→Emscripten→Unity Input System→`PlayerCommand` 경로를 증명하지만, 물리 컨트롤러 연결·장치별 버튼 표기·deadzone·조작감을 대체하지 않는다.
