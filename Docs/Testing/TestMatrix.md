...
- 플레이어 HUD의 초기/피해/사망 체력 snapshot과 bar 비율, 일반 방의 보스 panel 비표시, 보스 HUD의 초기/피해/2페이즈/격파 반영, 아홉 씬의 단일 HUD·session 참조와 실제 WebGL 배치 가독성.
...
- 게임패드 binding 구조, 합성 Input System 스틱·D-pad·버튼→의미 명령, WebGL 표준 가상 장치 연결→스틱·D-pad 해제·South 설치와 실패·West 교체 명령·Start pause/resume·Select 재시작. 물리 장치별 연결과 조작감은 수동 확인.
