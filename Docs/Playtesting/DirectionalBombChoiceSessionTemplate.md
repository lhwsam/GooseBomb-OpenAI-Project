...
- 공간 때문에 선택한 사례:
- 미니맵 `C/V/?` 이해와 길 찾기:
- Recovery 우회 선택과 `+2` 가치:

### 보스 목적지 예고·선행 설치

| 패턴 | ghost 첫 인지·예상 위치 | Execute 전 설치 셀·폭탄 | 회피 경로 | 실제 이동·폭발 결과 | 계획 근거/즉시 발언 |
|---:|---|---|---|---|---|
| 1 | | | | | |
| 2 | | | | | |
| 3 | | | | | |
...
- 첫 선행 설치 발생 패턴 또는 미발생:
- Recovery 이후 현재 위치 근접 설치 횟수:
- ghost·위험 셀·보스 본체 중 사용한 단서:
- 이동이 너무 잦음 / 적절함 / 너무 쉬움과 근거:
- 패턴 회피와 공격 준비가 연결됐는가:
- 관찰 사실:
- 참가자 핵심 발언:
- 진행자 해석(사실과 분리):
...
6. 직선 위력/난이도:
7. 광역 위험/쿨타임:
8. 보스 ghost의 첫 해석과 목적지 확신 시점:
9. Execute 전 선행 설치 여부와 근거/미발생 이유:
10. 회피와 공격 준비의 연결감, 이동 빈도·Recovery 시간:
11. 하나만 유지할 때 선택과 변경 제안:
...
| 미니맵이 스포일러 없이 길 찾기 피로를 줄이는가 | | | |
| Recovery 우회가 보스 준비를 위한 의미 있는 선택인가 | | | |
| 보스 목적지 ghost를 읽고 Execute 전에 선행 설치하는가 | | | |
| 보스 회피와 폭탄 준비가 한 판단으로 연결되는가 | | | |
| 이동 빈도와 Recovery가 퓨즈 계산에 적절한가 | | | |
